...
### Fixed
- **Scheduler not enabling**: Removed threading lock that caused deadlock, added debug logging
- **Better scheduler UI**: Shows time remaining, running status, and last execution info even when disabled

### Added
- **5 minute interval**: New `[5m]` option for testing auto-sync quickly
- More detailed scheduler status display
